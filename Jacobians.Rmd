...
But now consider this next model, which appears to place a standard normal prior on $e^x + \frac{x}{10}$. This seems like an obscure choice for a transform, but I promise I have my reasons^[In particular, I wanted a nonlinear transform whose domain and range both cover the entire real line (so either one can be normally distributed), whose derivative is strictly positive, and whose derivative is asymmetric about $x=0$ (so that a standard normal will be _obviously_ distorted, rather than just too light- or heavy-tailed). I wish I could think of a function that meets these constraints and also has a simple closed-form inverse. Feel free to send me suggestions.].
